...
export function PatientChat({ userData, socket }) {
  console.log("[PatientChat] Rendering with userData:", {
    userId: userData?._id,
    role: userData?.role,
  });
  ...
  const fetchAvailableDoctors = useCallback(async () => {
    try {
      ...
      if (!token) {
        ...
        console.error("[fetchAvailableDoctors] No token found");
        return;
      }
      const apiUrl = `${import.meta.env.VITE_API_URL}/api/messages/available-users`;
      console.log("[fetchAvailableDoctors] Fetching:", apiUrl);
      ...
      if (!response.ok) {
        ...
        setError(errorData.error || `Failed to fetch doctors (Status: ${response.status})`);
        console.error("[fetchAvailableDoctors] Error:", errorData);
        return;
      }
      const data = await response.json();
      console.log("[fetchAvailableDoctors] Fetched doctors:", data);
      setAvailableDoctors(data);
      setError("");
    } catch (err) {
      console.error("[fetchAvailableDoctors] Error:", err);
      setError("Failed to connect to server. Please try again.");
    }
  }, []);

  // Fetch messages for selected chat group
  ...
  const createChatGroup = useCallback(async (doctorId) => {
    try {
      const token = localStorage.getItem("token");
      if (!token) {
        setError("Authentication required. Please log in again.");
      ...
      console.log("[createChatGroup] Posting to:", apiUrl, "recipientId:", doctorId);
      const response = await fetch(apiUrl, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ recipientId: doctorId }),
      });
      if (!response.ok) {
        const errorData = await response.json();
  ...
  const handleDoctorSelect = useCallback(
    async (doctor) => {
      console.log("[handleDoctorSelect] Selected doctor:", {
        doctorId: doctor._id,
        chatGroupId: doctor.chatGroupId,
      });
      setSelectedDoctorId(doctor._id);
      setError("");
      if (doctor.chatGroupId) {
        setSelectedChatGroupId(doctor.chatGroupId);
      } else {
        const chatGroupId = await createChatGroup(doctor._id);
        if (chatGroupId) {
          setSelectedChatGroupId(chatGroupId);
          setAvailableDoctors((prev) =>
            prev.map((d) =>
              d._id === doctor._id ? { ...d, chatGroupId, lastMessage: null } : d
            )
          );
        } else {
  ...
  const sendMessage = useCallback(
    async (e) => {
      ...
      if (!selectedDoctorId || !selectedChatGroupId) {
        setError("Please select a doctor and initialize the chat");
      ...
      try {
        ...
        formData.append("recipientId", selectedDoctorId);
    ...
    [isSending, selectedChatGroupId, selectedDoctorId, newMessage, selectedFile]
  );
  ...
  useEffect(() => {
    ...
    socket.on("chatGroupUpdate", (updatedChatGroup) => {
      ...
      setAvailableDoctors((prev) =>
        prev.map((doctor) =>
          updatedChatGroup.participants.includes(doctor._id)
            ? {
                ...doctor,
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] bg-gray-100 rounded-xl shadow-2xl overflow-hidden">
      <div className="w-1/3 bg-white border-r border-gray-200 flex flex-col">
        ...
        {availableDoctors.length === 0 ? (
          <div className="p-6 text-gray-500 text-center">
            ...
            <p className="text-sm mt-1">No doctors found in the system.</p>
          </div>
        ) : (
            ...
            {availableDoctors.map((doctor) => (
              <li
                ...
                className={`p-4 rounded-lg cursor-pointer flex items-center space-x-4 transition-all duration-200 ${
                  selectedDoctorId === doctor._id ? "bg-blue-50 text-blue-800 shadow-md" : "hover:bg-gray-50"
                }`}
              >
                <img
                  src={
                    doctor.profilePicture
                      ? `${import.meta.env.VITE_API_URL}${doctor.profilePicture}`
                      : "/default-avatar.png"
                  }
                  alt={doctor.name}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500 truncate flex items-center">
                    {doctor.lastMessage ? (
                      <>
                        {String(doctor.lastMessage.senderId) === String(userData._id) && "You: "}
                        {doctor.lastMessage.attachment?.fileType?.startsWith("image/") && (
                          <svg
                        ...
                        {truncateMessage(doctor.lastMessage.content, doctor.lastMessage.attachment)}
                      </>
                    ) : (
                      "No messages yet"
                    )}
      ...
      <div className="w-2/3 flex flex-col bg-white">
        {selectedDoctorId && selectedChatGroupId ? (
          <>
            <div className="p-6 border-b border-gray-200 bg-gray-50">
              <h2 className="text-xl font-semibold text-gray-800">
                {availableDoctors.find((d) => d._id === selectedDoctorId)?.name || "Chat"}
              </h2>
            </div>
